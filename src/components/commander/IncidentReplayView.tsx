...
const EVENT_META: Record<ReplayEventType, { icon: AppIcon; color: string; label: string }> = {
  alert: { icon: Warning, color: 'bg-red-100 text-red-700 border-red-200', label: 'Alert' },
  ai_decision: { icon: Robot, color: 'bg-violet-100 text-violet-700 border-violet-200', label: 'AI Decision' },
  assignment: { icon: User, color: 'bg-blue-100 text-blue-700 border-blue-200', label: 'Assignment' },
  action: { icon: Lightning, color: 'bg-amber-100 text-amber-700 border-amber-200', label: 'Action' },
  failure: { icon: Warning, color: 'bg-orange-100 text-orange-700 border-orange-200', label: 'Failed Step' },
  escalation: { icon: ArrowCircleUp, color: 'bg-rose-100 text-rose-700 border-rose-200', label: 'Escalation' },
  resolution: { icon: CheckCircle, color: 'bg-emerald-100 text-emerald-700 border-emerald-200', label: 'Resolution' },
    ...
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <div className="flex items-center gap-2 text-xs text-ops-muted">
          <Broadcast size={14} weight="regular" aria-hidden />
          Incident Replay
        </div>
    ...
    {handoffSummary && (
      <div className="ops-card border-emerald-200 bg-emerald-50/50 p-4">
        <div className="flex items-center gap-2 text-sm font-semibold text-emerald-800">
          <Shield size={16} weight="regular" aria-hidden />
            ...
            return (
              <li key={event.id} className="relative pb-6 pl-6 last:pb-0">
                <span className={`absolute -left-[9px] top-1 flex h-4 w-4 items-center justify-center rounded-full border ${meta.color}`}>
                  <Icon size={10} weight="regular" aria-hidden />
                </span>
